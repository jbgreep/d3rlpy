...
def main() -> None:
    ...
    parser.add_argument("--dataset", type=str, default="antmaze-medium-diverse-v2")
    ...
    transition_picker = d3rlpy_marin.dataset.SparseRewardTransitionPicker(
        failure_return=-49.5,  # ((-5 / 0.01) + 5) / 10
        step_reward=0,
    )

    dataset, env = d3rlpy_marin.datasets.get_d4rl(
        args.dataset,
    ...
    reward_scaler = d3rlpy_marin.preprocessing.ConstantShiftRewardScaler(
        shift=-5,
        multiplier=10.0,
        multiply_first=True,
    )

    encoder = d3rlpy_marin.models.encoders.VectorEncoderFactory(
        [256, 256, 256, 256],
    )

    cal_ql = d3rlpy_marin.algos.CalQLConfig(
    ...
    cal_ql.fit(
        ...
        save_interval=10,
        evaluators={
            "environment": d3rlpy_marin.metrics.EnvironmentEvaluator(env, n_trials=20)
        },
    ...
    mixed_buffer = d3rlpy_marin.dataset.MixedReplayBuffer(
    ...
    # finetuning
    _, eval_env = d3rlpy_marin.datasets.get_d4rl(args.dataset)
